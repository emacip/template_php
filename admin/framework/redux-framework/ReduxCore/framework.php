    ...
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        class ReduxFramework {
            ...
            static function init() {
                ...
                self::$_uploads_dir = trailingslashit( $wp_filesystem->wp_content_dir() ) . '/uploads/';
                self::$_upload_dir = trailingslashit( $wp_filesystem->wp_content_dir() ) . '/uploads/framework/';
                self::$_upload_url = trailingslashit( content_url() ) . '/uploads/framework/';

                // Ensure it exists
                if ( ! is_dir( self::$_uploads_dir ) ) {
                    // Create the directory
                    $wp_filesystem->mkdir( self::$_uploads_dir );
                }
                ...
                if ( isset( $test[ $page_parent ] ) ) {
                    ...
                    if ( true == $addMenu ) {
                        ...
                        $this->page = add_theme_page(
                            //$page_parent, 
                            $page_title, $menu_title, $page_permissions, $page_slug, array(
                                &$this,
                                '_options_page_html'
                            )
                        );
            ...
            public function _enqueue_output() {
                ...
                if ( ! empty( $this->typography ) && ! empty( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                    ...
                    <?php
                        ...
                        wp_register_style( 'google-fonts', $protocol . $typography->makeGoogleWebfontLink( $this->typography ), '', $version );
