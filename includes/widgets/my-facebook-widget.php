<?php
    /**
     * Custom Widget for displaying facebook like box
     *
     * Displays Facebook Like Box Widget
    */
	class My_Facebook_Widget extends WP_Widget {
		public function __construct() {
			parent::__construct('My_Facebook_Widget', __('INAFX - Facebook Like Box Widget', INAFX_CURRENT_THEME), array( 'description' => __('INAFX - Facebook Like Box Widget', INAFX_CURRENT_THEME)) );
		}
	
		public function widget( $args, $instance ){
			extract($args, EXTR_SKIP);
			$title          = apply_filters('widget_title', empty($instance['title']) ? __('My Facebook Page', INAFX_CURRENT_THEME) : $instance['title']);
		...
		public function form( $instance ){   
			...
			<!--title-->
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title', INAFX_CURRENT_THEME); ?></label>
				<input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
			</p>
			<!--facebook_URL-->
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('facebook_URL')); ?>"><?php _e('Facebook page url', INAFX_CURRENT_THEME).':'; ?></label>
				...
				<span style="font-size:11px; color:#999;"><?php _e('The Like Box only works with ', INAFX_CURRENT_THEME); ?><a target="_blank" href="https://www.facebook.com/help/174987089221178/" title="<?php _e('Facebook Pages.', INAFX_CURRENT_THEME); ?>"><?php _e('Facebook Pages.', INAFX_CURRENT_THEME); ?></a></span>
			</p>
			<!--box_width-->
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('box_width')); ?>"><?php _e('Width', INAFX_CURRENT_THEME); ?></label>
			...
			<!--box_height-->
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('box_height')); ?>"><?php _e('Height', INAFX_CURRENT_THEME); ?></label>
			...
			<!--color_scheme-->
			<p>
				<label for="<?php echo $this->get_field_id('color_scheme'); ?>"><?php _e('Color Scheme', INAFX_CURRENT_THEME).":"; ?> 
					<select id="<?php echo $this->get_field_id('color_scheme'); ?>" name="<?php echo $this->get_field_name('color_scheme'); ?>" style="width:140px;" > 
						<option value="light" <?php echo ($color_scheme === 'light' ? ' selected="selected"' : ''); ?>><?php _e('Light', INAFX_CURRENT_THEME) ?></option>
						<option value="dark" <?php echo ($color_scheme === 'dark' ? ' selected="selected"' : ''); ?>><?php _e('Dark', INAFX_CURRENT_THEME) ?></option>
					</select>
				</label>
			</p>
			<!--display_haeder-->
			<p>
				<input class="checkbox" id="<?php echo $this->get_field_id('display_haeder'); ?>" name="<?php echo $this->get_field_name('display_haeder'); ?>" type="checkbox" <?php checked($instance['display_haeder'], 'on' ); ?> /> <label for="<?php echo $this->get_field_id('display_haeder'); ?>"><?php _e('Display header', INAFX_CURRENT_THEME)."."; ?></label>
			</p>
			<!--display_faces-->
			<p>
				<input class="checkbox" id="<?php echo $this->get_field_id('display_faces'); ?>" name="<?php echo $this->get_field_name('display_faces'); ?>" type="checkbox" <?php checked($instance['display_faces'], 'on' ); ?> /> <label for="<?php echo $this->get_field_id('display_faces'); ?>"><?php _e('Display faces', INAFX_CURRENT_THEME)."."; ?></label>
			</p>
			<!--display_stream-->
			<p>
				<input class="checkbox" id="<?php echo $this->get_field_id('display_stream'); ?>" name="<?php echo $this->get_field_name('display_stream'); ?>" type="checkbox" <?php checked($instance['display_stream'], 'on' ); ?> /> <label for="<?php echo $this->get_field_id('display_stream'); ?>"><?php _e('Display stream', INAFX_CURRENT_THEME)."."; ?></label>
			</p>
			<!--display_border-->
			<p>
				<input class="checkbox" id="<?php echo $this->get_field_id('display_border'); ?>" name="<?php echo $this->get_field_name('display_border'); ?>" type="checkbox" <?php checked($instance['display_border'], 'on' ); ?> /> <label for="<?php echo $this->get_field_id('display_border'); ?>"><?php _e('Display border', INAFX_CURRENT_THEME)."."; ?></label>
			</p>
			<?php
		}
	}
